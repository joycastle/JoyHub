...
class DeploymentCommandAppServiceTest {
    ...
    @Test
    void catalogPublishEnsuresApplicationBeforeDeployingRelease() {
        CatalogViewer viewer = new CatalogViewer("owner", null, null);
        AuditRequestContext audit = new AuditRequestContext("127.0.0.1", "test");
        DeployableApplication application = new DeployableApplication(
                1L, DeploymentMode.STATIC, "http://localhost/apps/demo-app/");
        org.springframework.test.util.ReflectionTestUtils.setField(application, "id", 3L);
        DeploymentStateService.PendingDeployment pending = new DeploymentStateService.PendingDeployment(
                3L, 11L, 21L, "demo-app", "v1", "a".repeat(64),
                "http://localhost/apps/demo-app/", new byte[]{1}, "app.zip");
        when(stateService.ensureApplication(1L, DeploymentMode.STATIC, viewer)).thenReturn(application);
        when(stateService.beginDeploy(3L, "v1", viewer)).thenReturn(pending);
        when(runnerClient.deploy(org.mockito.ArgumentMatchers.any(), eq(new byte[]{1}), eq("app.zip")))
                .thenReturn(new RunnerDeploymentResult(true, null, "deployed", "11", "http://static/demo-app/"));

        service.deployCatalogResource(1L, "v1", viewer, audit);

        verify(stateService).ensureApplication(1L, DeploymentMode.STATIC, viewer);
        verify(stateService).completeActivation(21L, "deployed", audit);
    }
}
