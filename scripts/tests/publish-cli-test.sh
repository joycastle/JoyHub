...
#
# The script runs local build-and-test (lint, typecheck, test, build, pack dry-run), bumps
...
#   - bin/npm stub: verifies the package dry-run command
...
BUN_EOF
  ...
  cat >"$repo/bin/npm" <<'NPM_EOF'
#!/usr/bin/env bash
set -euo pipefail
printf '%s\n' "$*" >> "${NPM_LOG_FILE:-/dev/null}"
[[ "$*" == "pack --dry-run" ]] || {
  echo "stub npm: unsupported: $*" >&2
  exit 1
}
NPM_EOF
...
EOF
  ...
  git -C "$repo" add cli/package.json cli/src/generated/pkg-info.ts \
                     scripts/publish-cli.sh bin/bun bin/npm bin/gh .gitignore
...
run_publish() {
  ...
  local cmd=(env -u GIT_DIR -u GIT_WORK_TREE -u GIT_INDEX_FILE
    ...
    GH_LOG_FILE="$repo/gh-stub.log" NPM_LOG_FILE="$repo/npm-stub.log")
  if [[ -n "$extra" ]]; then
...
grep -Fx "pack --dry-run" "$REPO7/npm-stub.log" >/dev/null \
  || fail "npm pack --dry-run was not invoked"
...
printf 'y\n' | env -u GIT_DIR -u GIT_WORK_TREE -u GIT_INDEX_FILE \
  ...
  GH_LOG_FILE="$REPO10/gh-stub.log" NPM_LOG_FILE="$REPO10/npm-stub.log" \
