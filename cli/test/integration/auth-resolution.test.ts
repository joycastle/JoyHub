 ...
 * are wired through the actual CLI subprocess: --flag > JOYHUB_* env >
...
async function seedConfig(home: string, registryUrl: string): Promise<void> {
  await mkdir(join(home, '.joyhub'), { recursive: true })
  await writeFile(
    join(home, '.joyhub', 'config.json'),
    JSON.stringify({ registry: registryUrl })
  )
}

// ---------------------------------------------------------------------------
// Token priority: --token > JOYHUB_TOKEN > stored
// ---------------------------------------------------------------------------

describe('auth resolution — token priority', () => {
  test('--token flag wins over JOYHUB_TOKEN env', async () => {
    ...
    const result = await runCli(
      ...
      { HOME: env.home, USERPROFILE: env.home, JOYHUB_TOKEN: 'sk_wrong_from_env' }
    )
    expect(result.exitCode).toBe(0)
    expect(result.stdout).toContain('flag-user')
  })

  test('JOYHUB_TOKEN env wins over stored token', async () => {
    const env = await createTempHome()
    registry = await startFakeRegistry({
      token: 'sk_from_env',
    ...
    const result = await runCli(
      ...
      { HOME: env.home, USERPROFILE: env.home, JOYHUB_TOKEN: 'sk_from_env' }
    )
    expect(result.exitCode).toBe(0)
...
// ---------------------------------------------------------------------------
// Registry priority: --registry > JOYHUB_REGISTRY > config.json
// ---------------------------------------------------------------------------

describe('auth resolution — registry priority', () => {
  test('--registry flag wins over JOYHUB_REGISTRY env', async () => {
    const env = await createTempHome()
    ...
    const result = await runCli(
      ...
      { HOME: env.home, USERPROFILE: env.home, JOYHUB_REGISTRY: registryB.url }
    )
    expect(result.exitCode).toBe(0)
    expect(result.stdout).toContain('a-user')
  })

  test('JOYHUB_REGISTRY env wins over config.registry', async () => {
    const env = await createTempHome()
    registry = await startFakeRegistry({
      token: 'sk_env',
    ...
    const result = await runCli(
      ['whoami', '--token', 'sk_env'],
      { HOME: env.home, USERPROFILE: env.home, JOYHUB_REGISTRY: registry.url }
    )
    expect(result.exitCode).toBe(0)
