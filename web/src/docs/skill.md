...
description: Use this when you need to search, inspect, install, or publish agent skills against a SkillHub registry. Prefer the official SkillHub CLI; ClawHub compatibility is available for OpenClaw clients.
---
...
> Important: Prefer the official `skillhub` CLI for registry workflows. SkillHub exposes a native CLI and API for auth, resolution, and download behavior. The ClawHub-compatible API and `/.well-known/clawhub.json` remain available for OpenClaw clients.
...
```bash
npx @astron-team/skillhub@latest install my-skill --registry https://skillhub.your-company.com
```
...
SkillHub supports these naming forms:

| SkillHub coordinate | Canonical slug for ClawHub-compatible clients |
|---|---|
...
```bash
npx @astron-team/skillhub@latest install my-skill
npx @astron-team/skillhub@latest install my-skill --version 1.2.0
npx @astron-team/skillhub@latest install team-name/my-skill
```
...
```bash
npx @astron-team/skillhub@latest search email
```

Use an empty query when you want a broad listing:

```bash
npx @astron-team/skillhub@latest search ""
```
...
skillhub search team-name/my-skill
```
...
skillhub install my-skill --version 1.2.0
skillhub install team-name/my-skill
```
