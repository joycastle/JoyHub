...
describe('CenterFeatureTour', () => {
  ...
  it('walks through the Agent center controls one at a time', () => {
    ...
    expect(onTargetChange).toHaveBeenLastCalledWith('quickBrowse')
    expect(screen.getByText('centerFeatureTour.centers.AGENT.quickBrowse.title')).toBeTruthy()
    fireEvent.click(screen.getByRole('button', { name: 'centerFeatureTour.next' }))
    expect(onTargetChange).toHaveBeenLastCalledWith('filters')
    expect(screen.getByText('centerFeatureTour.centers.AGENT.filters.title')).toBeTruthy()
    fireEvent.click(screen.getByRole('button', { name: 'centerFeatureTour.next' }))
